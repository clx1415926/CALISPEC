Vicuna_PATH=/root/paddlejob/workspace/env_run/CaliSpec/pretrained/Meta-Llama-3-8B-Instruct
Eagle_PATH=/root/paddlejob/workspace/env_run/CaliSpec/pretrained/EAGLE-LLaMA3-Instruct-8B
...
CUDA_VISIBLE_DEVICES=${GPU_DEVICES} python -m evaluation.inference_eagle2 --ea-model-path $Eagle_PATH --base-model-path $Vicuna_PATH --model-id ${MODEL_NAME}-eagle2-${torch_dtype} --bench-name $bench_NAME --temperature $TEMP --dtype $torch_dtype --top-k $topk --total-token $total_token
