...
def ea_forward(inputs, model, tokenizer, max_new_tokens, temperature=0.0):
    ...
    ##########
    input_ids, new_token, step, accept_length_list, all_similarities_list = model.eagenerate(
        torch.as_tensor(input_ids).cuda(),
        temperature=temperature,
        max_new_tokens=max_new_tokens,
        log=True
    ...
    ##########
    # --- 添加打印逻辑 ---
    print("\n--- Debug: Acceptance Lengths vs. Similarities ---")

    num_steps = len(accept_length_list)
    for i in range(num_steps):
        print(f"Step {i+1}:")
        print(f"  Acceptance Length: {accept_length_list[i]}")

        if i < len(all_similarities_list):
            sims_formatted = [f"{s:.4f}" for s in all_similarities_list[i]]
            print(f"  Avg Similarities per Draft Depth: {sims_formatted}")
        else:
            print("  Similarities: (data not available for this step)")
