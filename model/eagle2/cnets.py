...
class Model(nn.Module):
    ...
    def topK_genrate(self, hidden_states, input_ids, head, logits_processor):
        ...
        ##########
        # [NEW] 2. 记录第一层 (Depth 0) 的特征
        parent_hidden_state = last_hidden # Shape: [1, 4096]
        draft_embeddings = self.embed_tokens(input_ids[0]) # Shape: [top_k, 4096]
        sims = F.cosine_similarity(parent_hidden_state, draft_embeddings, dim=1)
        self.draft_similarities.append(sims.mean().item()) 

        # [FIXED] 关键修正：将 hidden state 扩展为 [top_k, 4096]，防止后续索引报错
        expanded_hidden = parent_hidden_state.repeat(top_k, 1)

        self.trace_log.append({
            'depth': 0,
            'hidden': expanded_hidden.detach().cpu(), # [FIXED] 使用扩展后的 hidden
            'embed': draft_embeddings.detach().cpu(),     
            'ids': input_ids[0].detach().cpu(),           
            'parent_idx': None,                           
            'self_idx': torch.arange(top_k).cpu()         
        })
        ##########
        ...
        for i in range(depth):
            ...
            ##########
            parent_states = input_hidden.squeeze(0) # shape [top_k, 4096]
            draft_embeddings = self.embed_tokens(input_ids[0]) # shape [top_k, 4096]
            sims = F.cosine_similarity(parent_states, draft_embeddings, dim=1)
            self.draft_similarities.append(sims.mean().item()) 
            
            # [NEW] 3. 记录后续层 (Depth > 0) 的特征
            self.trace_log.append({
                'depth': i + 1,
                'hidden': parent_states.detach().cpu(),   
                'embed': draft_embeddings.detach().cpu(), 
                'ids': input_ids[0].detach().cpu(),       
                'parent_idx': out_ids.detach().cpu(),       
                'self_idx': torch.arange(top_k).cpu()       
            })
            ##########
